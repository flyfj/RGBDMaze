...
for i = 1:length(data_params.datapath)
    ...
    switch kdes_params.kdes.type
         case {'gradkdes', 'lbpkdes', 'rgbkdes', 'nrgbkdes'}
              % read an image (changed from txt to mat)
              if strcmp(data_params.datapath{i}(end-2:end), 'mat'); I = load(data_params.datapath{i}); else; I = imread(data_params.datapath{i}); end
         ...
         case {'gradkdes_dep', 'lbpkdes_dep'}
              % read a depth map
              I = load(data_params.datapath{i}); %imread(data_params.datapath{i});
              % normalize depth values to meter
              I = double(I.refinedDepth)/1000;
