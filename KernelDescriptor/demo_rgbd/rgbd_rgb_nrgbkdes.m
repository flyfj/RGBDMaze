...
imsubdir = dir_bo(imdir);   % different object types
impath = [];

% label from image file name
rgbdclabel = [];    % class label, e.g. apple, orange
rgbdilabel = [];    % type label, e.g. apple1, apple2...
rgbdvlabel = [];    % instance label, e.g. apple1_1

subsample = 1;
label_num = 0;
for i = 1:length(imsubdir)
    % get all image paths under each class
    ...
    label_num = label_num + length(unique(rgbdilabel_tmp)); % every subclass as a training class
