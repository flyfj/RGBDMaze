...
%imdir = 'F:/Datasets/semantictest/processed/';
...
subsample = 1;
label_num = 0;
for i = 1:length(imsubdir)-40
    
    [rgbdilabel_tmp, impath_tmp] = get_im_label([imdir imsubdir(i).name '/'], '_depthcrop.png');
    %[rgbdilabel_tmp, impath_tmp] = get_im_label([imdir imsubdir(i).name '/'], '_depth.mat');
...
%data_params.savedir = ['../kdesfeatures/self_' 'gradkdes_dep'];
...
% do classification for category-level or instance-level
category = 1;
if category
   trail = 5;
   for i = 1:trail
