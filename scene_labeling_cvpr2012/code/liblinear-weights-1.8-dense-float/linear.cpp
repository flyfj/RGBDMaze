...
{
#ifdef DEBUG
	printf("Solver_MCSVM_CS::solve_sub_problem\n");
#endif
	int r;
	double *D;
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	for(i=0; i<l; i++)
	{
		...
		float *xi = prob->x[i];
		for(k=0; k<w_size; k++) {
                        xTx[i] += (double)(xi[k])*(double)(xi[k]);
                        w[k] += y[i]*alpha[2*i]*(double)xi[k];
		}
#else
		feature_node *xi = prob->x[i];
		while (xi->index != -1)
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<l; s++)
		{
			...
			if(inner_iter > 0) // update w
			{
				...
				xi = prob->x[i];
				for(k=0; k<w_size; k++)
                                        w[k] += sign*(z-alpha_old)*yi*(double)xi[k];
#else
				xi = prob->x[i];
				while (xi->index != -1)
				{
	...
	double Cp, double Cn)
{
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(k = 0; k < l; k++)
		{
			...
			x[k] *= y[k]; // x->value stores yi*xij
			xj_sq[j] += C[GETI(k)]*val*val;
		}
#else
//		x = prob_col->x[j];
//		while(x->index != -1)
...
//					}
				...
				if(num_linesearch == 0)
				{
					...
					{
						if(b[k] > 0)
							loss_old += C[GETI(k)]*b[k]*b[k];
						double b_new = b[k] + d_diff*(double)x[k];
						b[k] = b_new;
						if(b_new > 0)
							loss_new += C[GETI(k)]*b_new*b_new;
					}
				}
				else
				{
	...
	double Cp, double Cn)
{
	...
	for(j=0; j<w_size; j++)
	{
		...
		{
			if(y[k] == -1)
				xjneg_sum[j] += C[GETI(k)]*(double)x[k];
		}
#else
//		while(x->index != -1)
//		{
...
//		}
	...
	while(newton_iter < max_newton_iter)
	{
		...
		for(s=0; s<active_size; s++)
		{
			...
			for(k = 0; k < l; k++)
			{
                                Hdiag[j] += (double)x[k]*(double)x[k]*D[k];
                                tmp += (double)x[k]*tau[k];
			}
#else
//			while(x->index != -1)
//			{
