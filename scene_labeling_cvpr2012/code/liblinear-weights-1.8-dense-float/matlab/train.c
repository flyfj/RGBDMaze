...
{
	mexPrintf(
	"Usage: model = train(weight_vector, training_label_vector, training_instance_matrix, 'liblinear_options', 'col', 'variables to delete');\n"
	" ( xren ) : liblinear will delete the passed variables once copied to internal storage\n"
#ifdef _DENSE_REP
...
int read_problem_sparse(const mxArray *weight_vec, const mxArray *label_vec, const mxArray *instance_mat)
{
...
#endif
	size_t elements;
	int max_index, label_vector_row_num, weight_vector_row_num;
	float *samples;
	double *labels, *weights;
	...
	*/
	...
	prob.x = Malloc(float *, prob.l);
        ...
        use_existing_space=(col_format_flag && bias<0);
	...
	for(i=0;i<prob.l;i++)
        {
                prob.x[i] = &x_space[i*(max_index+(prob.bias>=0))];
